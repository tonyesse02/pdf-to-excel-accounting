...
def parse_amount(amount_str: str) -> float:
    ...
    if ',' in amount_str and '.' in amount_str:
        ...
        if len(parts) == 2 and len(parts[1]) == 2:
            # Probabilmente decimale (1000,50 o 1.000,50)
            amount_str = amount_str.replace('.', '').replace(',', '.')
        else:
            # Probabilmente migliaia in formato US (1,000)
            amount_str = amount_str.replace(',', '')
    elif '.' in amount_str:
        # Solo punto: potrebbe essere decimale (US) o migliaia
        parts = amount_str.split('.')
        if len(parts[-1]) != 2:  # Non è decimale (non ha 2 cifre dopo)
            amount_str = amount_str.replace('.', '')
...
def normalize_transaction(tx: dict) -> dict:
    """
    Normalizza una transazione: converte importi, standardizza nomi colonne.
    
    Args:
        tx: Transazione (dict)
        
    Returns:
        dict: Transazione normalizzata
    """
    normalized = {}
    ...
    for key, value in tx.items():
        ...
        # Mapping di colonne comuni
        if any(x in key_lower for x in ['data', 'date', 'fecha']):
            normalized['Data'] = str(value) if value else ''
        elif any(x in key_lower for x in ['ora', 'time', 'hora', 'orario']):
            normalized['Ora'] = str(value) if value else ''
        elif any(x in key_lower for x in ['codice', 'code', 'id', 'transazione']):
            normalized['Codice'] = str(value) if value else ''
        elif any(x in key_lower for x in ['descrizione', 'description', 'desc', 'causale']):
            normalized['Descrizione'] = str(value) if value else ''
        elif any(x in key_lower for x in ['importo', 'amount', 'ammontare', 'valore']):
            normalized['Importo'] = parse_amount(str(value)) if value else 0.0
        elif any(x in key_lower for x in ['commissione', 'commission', 'fee', 'tassa']):
            normalized['Commissioni'] = parse_amount(str(value)) if value else 0.0
        elif any(x in key_lower for x in ['stato', 'status', 'statut']):
            normalized['Stato'] = str(value) if value else ''
        else:
            # Colonne non riconosciute le manteniamo come sono
...
def deduplicate_transactions(transactions: list) -> list:
    ...
    for tx in transactions:
        # Normalizza prima di processare
...
def create_output_file(transactions: list, output_path: Path, file_metadata: dict):
    ...
    if transactions:
        ...
        for tx in transactions:
            ...
            # Gestisci sia valori float che stringhe
            amount = tx.get('Importo', 0)
            if isinstance(amount, str):
                amount = parse_amount(amount)
            else:
                amount = float(amount) if amount else 0.0
            
            commission = tx.get('Commissioni', 0)
            if isinstance(commission, str):
                commission = parse_amount(commission)
            else:
